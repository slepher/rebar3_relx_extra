...
collect_applications(Release, Path) ->
    ...
    AppFileMap = collect_app_files(Path),
    X = foldl(fun({Name,Vsn,Type}, {Ok, Errs}) ->
                      NameStr = to_list(Name),
                      case maps:find(NameStr, AppFileMap) of
                          {ok, AppFiles} ->
                             case read_application_from_files(NameStr, AppFiles, Vsn, Incls) of
                                 {ok, A} ->
...
collect_app_files(Path) ->
    collect_app_files(Path, #{}).

collect_app_files([Dir|Path], Acc) ->
    Acc1 =
        case lists:reverse(filename:split(Dir)) of
            [_Ebin, AppWithVsn|_] ->
                [AppName|_] = string:split(AppWithVsn, "-"),
                FullAppName = filename:join(Dir, AppName ++ ".app"),
                case filelib:is_regular(FullAppName) of
                    true ->
                        Files = maps:get(AppName, Acc, []),
                        maps:put(AppName, [FullAppName|Files], Acc);
                    false ->
                        Acc
                end;
            _ ->
    ...
    %%       Dir, ExtRe, false, 
    %%       fun(AppFile, Acc1) ->
    %%               Basename = filename:basename(AppFile),
    %%               AppName = filename:rootname(Basename),
    %%               Files = maps:get(AppName, Acc1, []),
    %%               maps:put(AppName, [AppFile|Files], Acc1)
    %%       end, Acc),
    collect_app_files(Path, Acc1);
collect_app_files([], Acc) ->
    Acc.

read_application_from_files(NameStr, Files, Vsn, Incls) ->
    read_application_from_files(NameStr, Files, Vsn, Incls, no_fault).

read_application_from_files(NameStr, Vsn, [AppFile|T], Incls, FirstError) ->
    case file:consult(AppFile) of
        {ok, [{application, _Name, _Dict} = Term]} ->
            case parse_application(Term, AppFile, Vsn, Incls) of
		{error, {no_valid_version, {Vsn, OtherVsn}}} when FirstError == no_fault ->
		    NFE = {no_valid_version, {{"should be", Vsn}, 
					       {"found file", AppFile, OtherVsn}}},
		    read_application_from_files(NameStr, [AppFile|T], Vsn, Incls, NFE);
		{error, {no_valid_version, {Vsn, _OtherVsn}}} ->
                    read_application_from_files(NameStr, [AppFile|T], Vsn, Incls, FirstError);
		Res ->
		    Res
	    end;
        {ok, Other} ->
            FirstError1 = update_first_error(FirstError, {badly_formatted_application, AppFile, Other}),
            read_application_from_files(NameStr, [AppFile|T], Vsn, Incls, FirstError1);

        {error, Reason} ->
            FirstError1 = update_first_error(FirstError, {consult_file_failed, AppFile, Reason}),
            read_application_from_files(NameStr, [AppFile|T], Vsn, Incls, FirstError1)
    end;
read_application_from_files(Name, Vsn, [], _, no_fault) ->
    {error, {application_vsn, {Name,Vsn}}};
read_application_from_files(_Name, _Vsn, [], _, FirstError) ->
    {error, FirstError}.

update_first_error(no_fault, Error) ->
    Error;
update_first_error(FirstError, _) ->
    FirstError.
%%______________________________________________________________________
